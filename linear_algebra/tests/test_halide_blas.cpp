...
struct BLASTestBase
{
  ...
  bool compareMatrices(
      ...
      Scalar epsilon = 200 * std::numeric_limits<Scalar>::epsilon())
  {
    bool equal = true;
    for (int i = 0; i < N * N; ++i)
    {
...
struct BLASFloatTests : public BLASTestBase<float>
{
  ...
  L2_TEST(sgemv_notrans,
          cblas_sgemv(CblasColMajor, CblasNoTrans, N, N, alpha, A, N, x, 1,
                      beta, y, 1),
          hblas_sgemv(HblasColMajor, HblasNoTrans, N, N, alpha, A, N, x, 1,
                      beta, y, 1));
  L2_TEST(sgemv_trans,
          cblas_sgemv(CblasColMajor, CblasTrans, N, N, alpha, A, N, x, 1, beta,
                      y, 1),
  ...
  L3_TEST(sgemm_transB,
          cblas_sgemm(CblasColMajor, CblasNoTrans, CblasTrans, N, N, N, alpha,
                      A, N, B, N, beta, C, N),
          hblas_sgemm(HblasColMajor, HblasNoTrans, HblasTrans, N, N, N, alpha,
                      A, N, B, N, beta, C, N));
  L3_TEST(sgemm_transAB,
          cblas_sgemm(CblasColMajor, CblasTrans, CblasTrans, N, N, N, alpha, A,
                      N, B, N, beta, C, N),
...
struct BLASDoubleTests : public BLASTestBase<double>
{
  ...
  L2_TEST(dgemv_notrans,
          cblas_dgemv(CblasColMajor, CblasNoTrans, N, N, alpha, A, N, x, 1,
                      beta, y, 1),
          hblas_dgemv(HblasColMajor, HblasNoTrans, N, N, alpha, A, N, x, 1,
                      beta, y, 1));
  L2_TEST(dgemv_trans,
          cblas_dgemv(CblasColMajor, CblasTrans, N, N, alpha, A, N, x, 1, beta,
                      y, 1),
  ...
  L3_TEST(dgemm_transB,
          cblas_dgemm(CblasColMajor, CblasNoTrans, CblasTrans, N, N, N, alpha,
                      A, N, B, N, beta, C, N),
          hblas_dgemm(HblasColMajor, HblasNoTrans, HblasTrans, N, N, N, alpha,
                      A, N, B, N, beta, C, N));
  L3_TEST(dgemm_transAB,
          cblas_dgemm(CblasColMajor, CblasTrans, CblasTrans, N, N, N, alpha, A,
                      N, B, N, beta, C, N),
...
void test_scopies()
{
  ...
  float alpha = s.random_scalar();
  std::vector<float> ex(s.random_vector(2 * 8192));
  std::vector<float> ey(s.random_vector(2 * 8192));
  std::vector<float> ax(ex), ay(ey);

  float *x = &(ex[0]);
  float *y = &(ey[0]);
  hblas_scopy(2 * 8192, x, 1, y, 1);
}
